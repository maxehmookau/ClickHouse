...
find $DIR/src $DIR/contrib/**-cmake -type f  | grep -vE '*.md$' | head -n50
find $DIR/src $DIR/contrib/**-cmake -type f  | grep -vE '*.md$' | wc -l
echo TAILS
find $DIR/src $DIR/contrib/**-cmake -type f  | grep -vE '*.md$' | tail -n50
echo 'Hashes:'
find $DIR/src $DIR/contrib/**-cmake -type f  | grep -vE '*.md$' | xargs md5sum | awk '{ print $1 }' | sort | head -n50
echo TAILS
find $DIR/src $DIR/contrib/**-cmake -type f  | grep -vE '*.md$' | xargs md5sum | awk '{ print $1 }' | sort | tail -n50
